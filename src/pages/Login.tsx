...
import ProviderAuthButton from "../components/ProviderAuthButton";
import {
...
const Login = () => {
  ...
  const logInWithEmail = () => {
    ...
    loginUserWithEmailAndPassword(email, password).then((res) => {
      if (res.error) {
  ...
  return (
    <div className="bg-[#242427] min-h-screen flex flex-col xl:flex-row relative items-start xl:items-center justify-center dark:text-white">
      <a className="font-bold tracking-wider text-xl text-white brand p-5 absolute top-3 left-2 ">
        HoCost
      </a>
      <div className="hidden xl:flex w-[600px] p-5 mx-auto">
        <svg
      ...
      <div className="w-[90%] xl:w-[70%] mx-auto max-w-[480px] ">
        <h1 className="text-lg tracking-wider ">Sign in to HoCost</h1>
        <div className="flex flex-wrap mt-3 space-x-2">
          <p className="text-[#9CA3AF] text-sm"> Don't have an account?</p>{" "}
          <a className="text-sm text-blue-500 underline">Get started</a>
        </div>
        ...
        <div className="mt-6 space-y-6">
          ...
          <button
            ...
            className="bg-blue-500 hover:bg-blue-500/80 transition-all w-full p-2 rounded-md"
          >
            Login
          </button>
        </div>
      ...
      {error ? (
        <Alert
          color="failure"
          className="absolute bottom-2 right-3 bg-red-500 rounded-md p-3"
        >
